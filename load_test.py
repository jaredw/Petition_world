...
def threadproc():
    ...
    print "Thread started: %s" % threading.currentThread().getName()

    while not quitevent.isSet():
            # HTTP requests to exercise the server go here
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            h = httplib.HTTPConnection('6.latest.show-your-vote.appspot.com', 80)
            h.request("GET", "/vote?skin=mini")
            if h.getresponse().status != 200:
                print "Response not OK"
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    print "Thread finished: %s" % threading.currentThread().getName()
